...
class PANEL_2d_views(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if len(elv_scenes) < 1:
            row.operator("fd_2d_views.create_2d_views",text="Prepare 2D Views",icon='SCENE_DATA')
        else:
            row.operator("fd_2d_views.create_2d_views",text="",icon='FILE_REFRESH')
            row.operator("fd_2d_views.render_2d_views",text="Render Selected Scenes",icon='SCENE_DATA')
        ...
        if len(image_views) > 0:
            ...
            panel_box.operator('fd_2d_views.create_pdf',text="Create PDF",icon='FILE_BLANK')
...
class MENU_elevation_scene_options(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("fd_2d_views.create_new_view",text="Create Snap Shot",icon='SCENE')
        layout.separator()
...
class LIST_2d_images(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        layout.operator('fd_2d_views.view_image',text="",icon='RESTRICT_VIEW_OFF',emboss=False).image_name = item.name
        layout.operator('fd_2d_views.delete_image',text="",icon='X',emboss=False).image_name = item.name
        
        
class OPERATOR_clear_2d_views(bpy.types.Operator):   
    bl_idname = "fd_2d_views.clear_2d_views"    
    ...
    def execute(self, context):
        for scene in bpy.data.scenes:
            if scene.mv.elevation_scene or scene.mv.plan_view_scene:
                if not scene.mv.plan_view_scene:
                    bpy.data.groups.remove(bpy.data.groups[scene.name], do_unlink=True)
...
class OPERATOR_create_2d_views(bpy.types.Operator):
    bl_idname = "fd_2d_views.create_2d_views"    
    bl_label = "Create 2d Views"
    bl_description = "Create 2D Views"
    bl_options = {'UNDO'}
...
#     
    ...
    def create_elv_view_scene(self, context, wall):
        wall_name = wall.obj_bp.mv.name_object
        wall_bp = wall.obj_bp
        wall_width = wall.obj_x.location.x
        wall_height = wall.obj_y.location.y
        wall_depth = wall.obj_z.location.z
        groups = bpy.data.groups
                
        grp = groups[wall_name] if wall_name in groups else groups.new(wall_name)
        elv_scene = self.create_new_scene(context, grp, wall_bp)
        self.group_children(grp, wall_bp)
        
        wall_mesh = utils.create_cube_mesh(wall_name, (wall_width, wall_height, wall_depth))
        wall_mesh.parent = wall_bp
        grp.objects.link(wall_mesh)
        
        instance = bpy.data.objects.new(wall_name + " "  + "Group Instance" , None)
        elv_scene.objects.link(instance)
        instance.dupli_type = 'GROUP'
        instance.dupli_group = grp
        
        elv_scene.world = self.main_scene.world
        self.link_dims_to_scene(elv_scene, wall_bp)
        self.add_text(context, wall)
        
        camera = self.create_camera(elv_scene)
        camera.rotation_euler.x = math.radians(90.0)
        camera.rotation_euler.z = wall_bp.rotation_euler.z   
    ...
    def execute(self, context):
        ...
        bpy.ops.fd_2d_views.clear_2d_views()
        
#FOR CREATING VIEWS NOT ASSIGNED TO A WALL   
...
class OPERATOR_render_2d_views(bpy.types.Operator):
    bl_idname = "fd_2d_views.render_2d_views"
    ...
    def render_scene(self, context, scene):
        ...
        print("context.scene: ", context.screen.scene)
        
#         scene.mv.opengl_dim.gl_default_color = (0.1, 0.1, 0.1, 1.0)
#         rd = scene.render
#         rl = rd.layers.active
#         fs_settings = rl.freestyle_settings
#         
...
#         rl.use_pass_z = False
#         fs_settings.crease_angle = 2.617994
#         
...
#             rd.engine = 'CYCLES'
#         
#         if scene.mv.render_type_2d_view == 'VIEWPORT':
#             print("Render OpenGL...")
...
#         while not os.path.exists(img_path + ".jpg"):
#             time.sleep(0.1)
#         
#         print("Loading into OpenGL memory...")
...
class OPERATOR_view_image(bpy.types.Operator):
    bl_idname = "fd_2d_views.view_image"
    bl_label = "View Image"
...
class OPERATOR_delete_image(bpy.types.Operator):
    bl_idname = "fd_2d_views.delete_image"
    bl_label = "View Image"
...
class OPERATOR_create_pdf(bpy.types.Operator):
    bl_idname = "fd_2d_views.create_pdf"
    bl_label = "Create PDF"
...
def register():
    ...
    bpy.utils.register_class(OPERATOR_clear_2d_views)
    bpy.utils.register_class(OPERATOR_create_2d_views)
...
def unregister():
    ...
    bpy.utils.unregister_class(OPERATOR_clear_2d_views)
    bpy.utils.unregister_class(OPERATOR_create_2d_views)
