...
class VIEW3D_PT_sketchfab(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        row = box.row()
        row.prop(props,'tabs',expand=True)
        
        if props.tabs == 'EXPORT':
            col = box.column()
        ...
        if sf_state.uploading:
            row.operator("export.sketchfab", text="Uploading %s" % sf_state.size_label,icon='FILE_REFRESH')
        else:
            row.operator("export.sketchfab",icon='URL')
            
        model_url = sf_state.model_url
        if model_url:
            row = box.row()
            row.scale_y = 1.5
            row.operator("wm.url_open", text="View Online Model", icon='RESTRICT_VIEW_OFF').url = model_url
...
class SketchfabProps(bpy.types.PropertyGroup):
    ...
    tabs = EnumProperty(
           name="Tabs",
           items=(('EXPORT', "Export Options", "Export Options"),
                  ('ACCOUNT', "Account Information", "Show Account Information")),
           description="Determines which lamps are exported",
           default='EXPORT',
           )
...
class SketchfabEmailToken(bpy.types.Operator):
    ...
    bl_label = "Enter your email to get a sketchfab api key"
    ...
    def draw(self,context):
        ...
        col.label("To get a Sketchfab API Key you need to create a free account on Sketchfab.")
        col.label("Visit www.sketchfab.com for more information.")
        col.label("If you have an account enter your account email address and click OK.")
